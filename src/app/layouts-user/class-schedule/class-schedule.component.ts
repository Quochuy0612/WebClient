...
export class ClassScheduleComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.classService.getClassDetail().subscribe((data :any[]) => {
      console.log(data);
    });//gọi hàm getClassdetail mỗi khi reload;
  }
    ...
    this.classDetail.forEach(element => {//chạy vòng lạp để xác định class deatil của class cần signup
      if (element._classId == classId) {//true/false
        ...
        if (this.qty >= 0) {//nếu sốlượng sau khi trừ >=0 thì mới thực hiện
          this.classService.signupToLearn(this.userId, this.qty, classId).subscribe((res: HttpResponse<any>) => {
            console.log(res);
  ...
  clickClass() {
    this.classService.getClass(this.selectedView).subscribe((data: Level) => {
      this.clases = [data.class];
  ...
  check() {
    this.class.forEach(element => {
      console.log(element)
      this.idTeacher = element.lecturers;
